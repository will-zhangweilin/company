...
if($action=="login"){
	...
	if(!$metinfo_admin_name||!$metinfo_admin_pass){
		if($admin_index){
			...
			$re_url=$re_url?$re_url:$_SERVER[HTTP_REFERER];
		...
		if(!$admincp_ok){
			if($admin_index){
				...
				if(!$re_url){
					...
					$HTTP_REFERERs=explode('?',$_SERVER[HTTP_REFERER]);
					$admin_file_len1=strlen("/$met_admin_file/");
					$admin_file_len2=strlen("/$met_admin_file/index.php");
					if(strrev(substr(strrev($HTTP_REFERERs[0]),0,$admin_file_len1))=="/$met_admin_file/"||strrev(substr(strrev($HTTP_REFERERs[0]),0,$admin_file_len2))=="/$met_admin_file/index.php"||!$HTTP_REFERERs[0]){

...
global $lang,$lang_chchinese,$met_ch_mark,$met_ch_lang,$met_langok,$met_url,$index_url,$met_index_url,$met_lang_mark,$met_waplink,$lang_wap,$met_wap_tpb,$met_wap_url,$met_wap,$met_index_type,$navurl;
if($met_lang_mark){
switch($type){
case 1:
$metinfo='';
...
return $metinfo;
}elseif($met_ch_lang and $lang==$met_ch_mark){
$metinfo="<a class=fontswitch id=StranLink href=\"javascript:StranBody()\">".$lang_chchinese."</a><script src=\"".$met_url."js/ch.js\" type=\"text/javascript\"></script>";
return $metinfo;
}
}
...
function methtml_getarray($mark,$type,$order,$module,$listmx=-1,$para=0,$categoryname=0,$marktype=0,$txtmax=0,$descmax=0){
	...
	if($modulenunm==6){
		$query = "SELECT $select FROM $table where lang='$lang' $access_sql order by top_ok desc,no_order desc,addtime desc limit 0, $listmx";	
	}
	else{
...
function methtml_online(){
	global $met_online_type,$navurl,$met_onlineleft_left,$met_onlineright_right,$met_onlineleft_top,$met_onlineright_top,$lang;
	$metinfo='';

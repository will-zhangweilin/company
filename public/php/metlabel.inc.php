...
function metlabel_html5($closure=1,$iehack=1){
	global $met_title,$show,$m_now_year,$navurl,$met_js_access,$met_skin_css,$img_url,$met_webname,$metcms_v,$appscriptcss;
				...
				foreach($nav_list2[$val['id']] as $key=>$val2){
					...
					$metinfo.="<p>";
						foreach($nav_list3[$val2['id']] as $key=>$val3){
							$metinfo.="<a href='{$val2[url]}' title='{$val2[name]}' {$val2[new_windows]}>{$val2[name]}</a>";
						}
					$metinfo.="</p>";
					}
					$metinfo.="</dd>";
...
function metlable_lang($dt,$tp=1){
	global $methtml_sethome,$methtml_addfavorite,$index_hadd_ok;
...
function metlabel_list($listtype='text',$mark,$type,$order,$module,$time=0,$titleok=1,$bian=1,$listmx,$txtmax){
	...
	$listarray=methtml_getarray($mark,$type,$order,$module,$listmx,'','','',$txtmax);
...
if($titleok)$metinfo.="<h3 style='width:{$met_img_x}px;'><a href='{$val[url]}' title='{$val[title]}' {$metblank}>{$val[title]}</a></h3>";
			$metinfo.="</li>";
			}
			$metinfo.="</ol>";
			break;
...
//文章模块列表函数
function metlabel_news($time=1,$desc=0,$dlen,$dt=1){
...
//产品模块列表函数
function metlabel_product($z,$w,$l){
...
function sitemaplist(){
	...
	foreach($nav_listall as $key=>$val){
		...
		if($val[module]!=10 && $val[module]!=11 && $no1ok && $no2ok){

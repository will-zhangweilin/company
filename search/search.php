...
if($searchword==""){
$search_list[0][title]="<em>{$lang_SearchInfo1}</em>";
...
break;
	...
	foreach ($search_list as $key=>$value){
		...
		$listok=1;
		if($searchtype!=1){
			if(stripos($value['content'],$searchword)===false)$listok=0;
		}
		if($listok){
...
$search_list[0][title]="{$lang_SearchInfo3}[<em>$searchword</em>]{$lang_SearchInfo4}";
$search_list[0][updatetime]=$m_now_date;  
$search_list[0][url]=$index_url; 
}
